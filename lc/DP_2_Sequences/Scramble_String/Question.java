...
public class Question {
    ...
    public boolean isScramble(String s1, String s2) {
        ...
        for (int len = 2; len <= strLen; len++) { // 即组成左边scramble+右边scramble可能的长度从2一直到strLEn
        	for (int i = 0; i <= strLen - len; i++) { // 关于边界：i <= strLen - len; 举例令len为2，strLen为8，则i最远只能走到index==6，带入对比系数可得表达式。
        		for (int j = 0; j <= strLen - len; j++) {
        			for (int k = 1; k < len; k++) { //k可以把len劈len - 1刀，即k可以出现在len内部的（1~len - 1）位置上
        				...
        				//【注】要时刻分清k和len的区别：len是当前的字符串长度，k表示的是在len这么长的串里劈那1刀的位置（此处的表达式里不应该再出现strLen了）
        				//result[i][j][k] && result[i + k][j + k][len - k]：在i和j的第k个位置之后劈开，左与左([起点i][长度k]与[起点j][长度k])为真&&右与右([起点i+k][长度len-k]与[起点j+k][长度len-k])为真（看图）
        				//result[i][j][k] && result[i + k][j + k][len - k]：在i的第k个位置之后,j的第len-k劈开，左与右([起点i][长度k]与[起点j+len-k][长度k])为真&&右与左([起点i+k][长度len-k]与[起点j][长度len-k])为真（看图）
    ...
    //在leetcode实际效果中，recursion比DP要快(DP:283ms; Recursion: 244ms)
    ...
    private boolean helper(String s1, String s2) {
    	...
    	Arrays.sort(s1Char);
    	String s1Sort = new String(s1Char); // 【注】s1Char.toString();这样写是错的！s1Char.toString()的实际内容是hashcode而不是s1Char的内容！
