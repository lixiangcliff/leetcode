...
public class Question {
	...
	public static void main(String[] args) {
		...
		int k = 100;
		System.out.println(q.maxProfit2(k, prices));
	}
	...
	//local[i][j]前i天最多完成j次交易,并且第最后一次卖出发生在第i天的最大profit; 
	//res[i][j]前i天最多完成j次交易最大profit
        ...
        for (int i = 0; i <= prices.length; i++) {
        	for (int j = 0; j <= k && j <= i; j++) {
        		if (i == 0 || j == 0) {
        			...
        			int diff = i > 1 ? prices[i - 1] - prices[i - 2] : 0; //diff表示第i - 1天买，第i天卖的profit
        			//【注】local[i][j]表示必须在第i天进行第j次卖出，那么按照第j次交易的买入的时间，分成两种情况：
        			//1.买入时间为第i - 1天或者第i天：则在第i-1天的时候已经进行过j-1次交易了，如果diff > 0，就在第i-1天买，否则就在第i天买 ，即res[i - 1][j - 1] + Math.max(diff, 0)
        			//2.买入时间为第i - 1天之前的某一天：那么先得到如果在第i-1天完成第j次卖出，现在我想把这第j次的卖出推迟到i天，则需要把“由于在第i-1天卖出第j次的结果”消除掉，即加上diff(无论diff是否大于0)，即local[i - 1][j] + diff
        			local[i][j] = Math.max(res[i - 1][j - 1] + Math.max(diff, 0), local[i - 1][j] + diff); // 不好理解，研究一下。
        			res[i][j] = Math.max(res[i - 1][j], local[i][j]);
        		}
        	}
        }
        return res[prices.length][k];
    }
	
	//input太大时过不了。怀疑代码本身也有问题。
	//res[i][j]前i天最多完成k次交易最大profit
	//res[i][j] = max(max(res[pos - 1][j - 1] + prices[i] - prices[pos]，其中pos从[0, i]), res[i - 1][j])
        ...
        for (int i = 0; i <= prices.length; i++) {
        	for (int j = 0; j <= k && j <= i; j++) {
        		if (i == 0 || j == 0) {
        			...
        			int max = 0;
        			while (pos < i) {
        				max = Math.max(res[pos - 1][j - 1] + prices[i - 1] - prices[pos - 1], max);
        				pos++;
        			}
